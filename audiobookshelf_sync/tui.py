...
from audiobookshelf_sync.queue import QUEUE_FILE, add_pending_item, load_queue, save_queue


class ResultItem(ListItem):
    def __init__(self, result: BookSearchResult) -> None:
        super().__init__(Label(format_result(result)))
        self.result = result
...
class SearchQueueApp(App[None]):
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        self.set_status(f"Found {len(results)} result(s). Press a to queue the selected book.")
    ...
    async def refresh_queue(self) -> None:
        ...
        for item in queue.items:
            await queue_list.append(
                QueueListItem(f"{item.status}: {item.title} - {item.author}")
            )
...
def format_result(result: BookSearchResult) -> str:
    parts = [result.title, result.author, result.library_name]
